                ...
                <div className="w-full max-w-md bg-[#13151c] border border-[#1f222c] rounded-2xl p-8 shadow-2xl">
                    ...
                    {onboardingStep === 'NAME' && (
                        <form onSubmit={handleNameSubmit} className="space-y-4">
                            ...
                            <input type="text" autoFocus placeholder="Introduce yourself..." className="w-full bg-[#181a24] border border-[#232635] rounded-xl px-4 py-3 text-xs outline-none text-white focus:border-[#a855f7]" value={userData.name} onChange={(e) => setUserData({...userData, name: e.target.value })} />
                    ...
                    {onboardingStep === 'PREFERENCES' && (
                        <div className="space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" onClick={() => handlePreferenceSelect('theme', 'cyberpunk_neon')} className={`py-2.5 rounded-xl border text-xs font-bold transition-all ${userData.theme === 'cyberpunk_neon' ? 'border-[#a855f7] bg-[#a855f7]/10 text-white shadow-lg' : 'border-[#232635] bg-[#181a24] text-slate-400 hover:border-slate-700'}`}>🌙Cyberpunk Neon</button>
                                <button type="button" onClick={() => handlePreferenceSelect('theme', 'minimal_clean')} className={`py-2.5 rounded-xl border text-xs font-bold transition-all ${userData.theme === 'minimal_clean' ? 'border-[#a855f7] bg-[#a855f7]/10 text-white shadow-lg' : 'border-[#232635] bg-[#181a24] text-slate-400 hover:border-slate-700'}`}>☀️Minimal Clean</button>
                            </div>
                            <div className="space-y-2">
                                {['💻 Software Engineering', '🧬 Art & Science Division', '📊 Core Data Analyst'].map(dept => {
                                    ...
                                    return (
                                        <button type="button" key={dept} onClick={() => handlePreferenceSelect('department', standardizedDept)} className={`w-full text-left px-4 py-3 rounded-xl text-xs transition-all border ${isSelected ? 'border-[#a855f7] bg-[#a855f7]/5 text-white font-medium' : 'bg-[#181a24] border-[#232635] text-slate-300 hover:border-[#a855f7]/50'}`}>{dept}</button>
                                    );
                                })}
                            </div>
                        </div>
        ...
        <div className="flex flex-col h-screen w-screen font-sans overflow-hidden select-none bg-[#0d0e12] text-slate-200">
            <header className="h-14 border-b border-[#1f222c] bg-[#13151c] px-6 flex justify-between items-center text-sm shrink-0">
                ...
                <div className="text-xs flex gap-4 font-medium items-center text-slate-400">
                    ...
                    <button onClick={() => { setSiteIdea(""); setOnboardingStep('NAME'); }} className="text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1 rounded transition-colors">
                        Reset Flow
                    </button>
                </div>
            </header>
