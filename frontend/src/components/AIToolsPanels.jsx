    ...
    return ( <
        ...
        div className = "space-y-2" > {
            templates.map((tmpl, idx) => ( <
                ...
                className = "w-full text-left p-2.5 bg-[#181a24] hover:bg-[#1e212f] border border-[#232635] text-[11px] text-slate-300 rounded-xl transition-all" > 💡{ tmpl } <
                /button>
            ))
        } <
        /div> < /
...
export function CodeExport({ pages }) {
    ...
    return ( <
        ...
        className = "w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-2.5 px-4 rounded-xl text-xs transition-colors shadow-lg" > 📦Export Blueprint Configuration <
        /button> < /
        div >
    );
}
